...
use App\Http\Controllers\AuctionProductBidController as ControllersAuctionProductBidController;
 ...
 */
 ...
 Route::get('/assign-super-admin', function () {
    // 1. Find the user
    $user = User::where('email', 'studioequine@outlook.com')->first();
    ...
    // 3. Give all permissions to "Super Admin" role
    ...
    return 'Super Admin role and permissions assigned successfully to studioequine@outlook.com';
});
Route::get('/send-notification', [PushNotificationController::class, 'sendNotification']);

Route::get('/testing', function (){
    $template  = EmailTemplate::find(id: 1);
    return (new PushNotificationController)->sendBrowserNotification(1, 1, $template);
});
...
})->middleware("with-view-share-vars");
...
Route::middleware(['web'])->controller(HomeController::class)->group(function () {
    ...
    // Route::get('/static-pages/{type}', 'staticPages')->name('staticPages');
    Route::get('page/{page:slug}', 'get_page')->name('staticPages');

    Route::get('/track-your-order', 'trackOrder')->name('orders.track');
    // Route::get('/market-place', 'marketplace')->name('marketplace');

    Route::get('/abouts', 'abouts')->name('about');

    Route::get('/contacts', 'contact_us')->name('contact_us');
    Route::post('/send-enquiry', 'sendEnquiry')->name('send-enquiry');

    Route::get('my-addresses/', [AddressController::class, 'show_my_addresse'])->name("customer.addresses");
    Route::post('update-default-address/', [AddressController::class, 'addressMakeDefault'])->name("update-default-address");
    Route::post('delete-address/{id}', [AddressController::class, 'destroy'])->name("address.delete");

    Route::controller(AddressController::class)->group(function () {
        Route::post('/get-states', 'getStates')->name('get-state');
        Route::post('/get-cities', 'getCities')->name('get-city');
    });
    Route::get('/user-aggrement','download_pdf_file')->name('client-aggrement');
});
...
Route::controller(SearchController::class)->group(function () {
    ...
    Route::get('/category/{category:slug}', 'listingByCategory')->name('products.category');
    Route::get('/auction-category/{category:slug}', 'auctionListingByCategory')->name('auction.products.category');

    Route::get('/upcoming-auction-category/{category:slug}', 'upcomingAuctionListingByCategory')->name('upcoming-auction.products.category');

    Route::get('/products/best-selling', 'bestSelling')->name('products.bestSelling');
    Route::get('/products/{type}', 'index')->name('products.bestfeatured');
    Route::get('/market-place', 'index')->name('marketplace');
...
Route::controller(CompareController::class)->group(function () {
    ...
    Route::post('/compare/remove/{product}', 'remove')->name('compare.remove');
...
Route::group(['middleware' => ['user', 'verified', 'unbanned',"with-view-share-vars"]], function () {
    ...
    Route::controller(HomeController::class)->group(function () {
        ...
        Route::post('/user/update-password', 'passwordUpdate')->name('user.profile.update-password');
        Route::get('/user/change-password', 'change_password')->name('user.profile.change-password');
        Route::get('/apply-verification', 'verification_form')->name('apply-verification');
    });

    Route::get('bank-info', [CardController::class, 'bankInfo'])->name('bank-info');
    Route::post('bank-info', [CardController::class, 'saveBankInfo'])->name('save-bank-info');
    Route::post('become-seller', [ShopController::class,'submitBecomeSellerRequest'])->name('become.seller');
...
Route::group(['prefix' => 'checkout'], function () {
    Route::controller(CheckoutController::class)->group(function () {
        ...
        Route::get('/payment-select', 'payment_select')->name('checkout.payment-select');
...
Route::group(['middleware' => ['user', 'verified', 'unbanned']], function () {
    ...
    Route::get('watchlist', [WishlistController::class,"index"])->name("wishlists.index");
    Route::get('auction-watchlist', [WishlistController::class,"getWatchlistAuctionProducts"])->name("wishlists.auction");
    ...
    Route::get('/bidded_products', [ControllersAuctionProductBidController::class,'index'])->name("bidded_products");
    Route::get('/bidded_products/chat/{bid}', [BidderChatController::class,'chatIndex'])->name("chat.index");
    Route::post('/get-update/chat/{chat}', [BidderChatController::class,'checkNewMsg'])->name("chat.updates");
    Route::get('/auction_purchase_history', [AuctionProductController::class,'purchase_history_user'])->name("auction_purchase_history");
...
Route::controller(BlogController::class)->group(function () {
    Route::get('/news', 'all_blog')->name('blog');
    Route::get('/news/{slug}', 'blog_details')->name('blog.details');
});
...
//Testing routes need to be removed when the software is deployed.
//pass like this system-log/2024-05-18
Route::get('/system-log/{date}', function ($date) {
  ...
  // Make Empty Laravel Log file
  // This route is need to remove when development complete
  Route::get('/system-clear-log/{date}', function ($date) {
    // Validate the date format (YYYY-MM-DD)
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $date)) {
        return response('Invalid date format. Please use YYYY-MM-DD.', 400);
    }
    ...
    if (file_exists($logFilePath)) {
        ...
        return response('Log file cleared successfully');
    } else {
        return response('Log file not found', 404);
    }
});
...
// Route::post('/xero/invoice', [XeroController::class, 'createInvoice'])->name('xero.invoice.create');
// Route::post('/xero/payment', [XeroController::class, 'recordPayment'])->name('xero.payment.record');


Route::post('/subscribe', [PushNotificationController::class, 'subscribe'])->name('subscribe.notification');
