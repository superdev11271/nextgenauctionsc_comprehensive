...
@section('panel_content')
    ...
    <div class="">
        ...
        @endif
        <form  id="auction-product-add-form" class=" form form-horizontal mar-top" action="{{ route('auction_product_store.seller') }}" method="POST"
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group row">
                                <label class="col-md-3 col-from-label aiz-">{{ translate('Auction No.') }}</label>
                                <div class="col-md-8">
                                    <select class="form-control aiz-selectpicker"  id="auctionSelect" onchange="updateLotNo()">
                                        <option value= "{{ $auctionNumber }}#new">New Auction</option>
                                        @foreach ($products as $product)
                                            <option value="{{ $product->auction_number }}#col">
                        ...
                        <div class="card-body">
                            <!-- <div class="form-group row">
                                <label class="col-md-3 col-from-label">{{ translate('Starting Bidding Price') }} <span
                                        class="text-danger">*</span></label>
                                <div class="col-md-9">
                                    <input type="number" id="starting_bid" lang="en" min="0" value="1" step="0.01"
                                        placeholder="{{ translate('Starting bidding price') }}" name="starting_bid"
                                        value="{{old('starting_bid')}}"
                                        class="form-control" required>
                                        <span id="starting_bid-error" class="text-danger"></span>
                                </div>
                            </div> -->
                            ...
                            @php
                            // Set Default auction time is data fount null
                            $daysToAdd = 2;
                            $start_date =  date('d-m-Y H:i:s', time());
                            $end_date =  (new DateTime('now +1 second'))->modify("+$daysToAdd days")->format('d-m-Y H:i:s');
                            @endphp
                        <div class="form-group row">
                            <label class="col-sm-3 control-label"
                                for="start_date">{{ translate('Auction Date Range') }} <span
                                    class="text-danger">*</span></label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control aiz-date-range" id="auction_date_range" name="auction_date_range"
                                    placeholder="{{ translate('Select Date') }}" data-time-picker="true"
                                    value="{{ old('auction_date_range') }}"
                                    data-past-disable="true" data-format="DD-MM-Y HH:mm:ss" data-separator=" to " autocomplete="off" required readonly>
                            ...
                            <!-- <div class="form-group row">
                                <label class="col-sm-3 control-label"
                                    for="estimate_start">{{ translate('Estimate Start') }} <span
                                        class="text-danger">*</span></label>
                                <div class="col-sm-9">
                                    <input type="text" id="estimate_start" placeholder="{{ translate('Enter Estimate Start') }}"
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" id="estimate_end" placeholder="{{ translate('Enter Estimate End') }}"
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card">
                        ...
                        <div id="category-error"  class="alert alert-danger d-none" role="alert">
                            Please select a product category, and the main category must be within the selected category.
                        </div>
                        <div class="card-body">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-6">
                                        <label class="aiz-switch aiz-switch-success mb-0">
                                            <input type="radio" name="shipping_type" value="free" @checked(old("shipping_type")=="free") checked>
                                            <span></span>
                                        </label>
                                    </div>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="form-group mb-3">
                                ...
                                <div class="input-group mt-2">
                                    <input type="text" class="form-control" id="picktime" name="pickup_time" required  value="{{ old('pickup_time',$lastProduct->pickup_time ?? '') }}" placeholder="{{ translate('time') }} (e.g., 1AM to 2PM)" oninput="validateTime(this)">
                                ...
                                <div class="input-group mt-2">
                                    <input type="text" class="form-control" name="pickup_address" required
                                         value="{{ old('pickup_address',$lastProduct->pickup_address ?? get_setting('contact_address',null,App::getLocale())) }}"
    ...
    <script type="text/javascript">
        ...
        function updateLotNo() {
            ...
            if (selectedValue[1] === 'new') {
                ...
                $('#pickdate').daterangepicker({
                    singleDatePicker: true,
                    startDate: "{{ $lastProduct->pickup_days ?? now()->format('d-m-Y') }}",  // Fallback to today's date
                    locale: {
                        format: 'DD-MM-YYYY'
                    },
                    minDate: moment(),
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        var startDate = moment.unix(response.product.auction_start_date);
