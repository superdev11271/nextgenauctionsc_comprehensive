    ...
    <section class="shop-section items-container  clearfix aos-init aos-animate" data-aos="fade-up">
        <div class="auto-container wow fadeInUp animated animated animated">
            <div class="sec-title">
                ...
                <h2>{{ translate('All Upcoming Auction') }}</h2>
            ...
            <div class="row mt-2">
                <div class="col-md-12">
                    <div class="d-flex align-items-center justify-content-end mb-2  action_switcher">
                        <button id="collectionBtn" class="theme-btn-one {{ activeRoute(['upcoming_auction_collection']) }}"
                            onclick="toggleButton('collectionBtn', 'upcoming_auction_collection')">Collection</button>
                        <button id="itemsBtn" class="theme-btn-one {{ activeRoute(['auction_products.upcoming','auction.products.category']) }}"
                            onclick="toggleButton('itemsBtn', 'auction_products.upcoming')">Items</button>
                    </div>
                </div>
                <div class="col-md-12">
    ...
    <script>
        ...
        function toggleButton(buttonId, route) {
            ...
            @php
                ...
                $collectionNavigate = route('upcoming_auction_collection', $mergedQuery);
                $itemNavigate = route('auction_products.upcoming', $mergedQuery);
            @endphp

            var collectionNavigate = "{!! $collectionNavigate !!}";
            var itemNavigate = "{!! $itemNavigate !!}";

            window.location.href = route === "upcoming_auction_collection" ? collectionNavigate : itemNavigate
        }
