                  ...
                  <div class="card-header py-3 d-flex justify-content-between align-items-center">
                            ...
                            <form method="GET" action="{{ url()->current() }}" class="form-inline">
                                <select name="status" class="form-control" onchange="this.form.submit()">
                                    ...
                                    <option value="won" {{ request('status') == 'won' ? 'selected' : '' }}>{{ translate('Won') }}</option>
                                    <option value="lost" {{ request('status') == 'lost' ? 'selected' : '' }}>{{ translate('Lost') }}</option>
                                </select>
                            </form>
                        </div>
                                        ...
                                        <tr class="cart-item">
                                            ...
                                            <td class="text-" style="vertical-align: middle;">
                                                <a href="{{ route('auction-product', $bid->product->slug) }}"
                                                    ...
                                                    <span class="  ml-1 text-break-custom">{{ Str::limit($bid->product->getTranslation('name'),12) }}</span>
                                                </a>
                                            </td>
                                            <td>{{ $bid->product->getFormattedAuctionNumber() }}</td>
                                           ...
                                           <td style="vertical-align: middle;">
                                                @if ($bid->product->sold_status == 'sold')
                                                    @if ($bid->product->sold_to == auth()->user()->id)
                                                        <div class="badge bg-success p-2 fs-13">
                                                            {{ translate('Won') }}
                                                        </div>
                                                    @else
                                                        <div class="badge bg-danger p-2 fs-13">
                                                            {{ translate('Lost') }}
                                                        </div>
                                                    @endif
                                                @endif
                                            ...
                                            <td class="text-center pr-0" style="vertical-align: middle;">
                                                @php
                                                    ...
                                                    $carts = get_user_cart_only_marketplace();

                                                    $cart_has_this_product = isProductInCart($bid->product_id);
                                                @endphp

                                                @if ($order != null)
                                                    <span
                                                        class="badge bg-success p-3 fs-12">{{ translate('Purchased') }}</span>
                                                @elseif ($bid->product->auction_end_date >= strtotime('now'))
                                                    <button type="button" class="btn btn-sm buy-now fw-600 rounded-0"
                                                        disabled>
                                                        Live
                                                    </button>
                                                @elseif ($bid->product->sold_status == null)
                                                    <button type="button" class="btn btn-sm buy-now fw-600 rounded-0"
                                                        disabled>
                                                        {{ translate('Under Review') }}
                                                    </button>
                                                @elseif ($bid->product->sold_status == 'sold' && $bid->product->sold_to != auth()->user()->id)
                                                    {{-- todo show other sold status --}}
                                                    <button type="button" class="btn btn-sm buy-now fw-600 rounded-0"
                                                        disabled>
                                                        {{ translate('Sold') }}
                                                    </button>
                                                @elseif($bid->product->sold_status == 'sold' && $bid->product->sold_to == auth()->user()->id)
                                                    ...
                                                    @elseif (count($carts) > 0 && !$cart_has_this_product)
                                                        <button type="button" class="btn btn-sm buy-now fw-600 rounded-0"
                                                            data-toggle="tooltip"
                                                ...
                                                {{-- Chat Icon --}}
                                                @if ($order == null && $bid->chats()->count() && $bid->product->sold_status == null && $bid->status == 'open')
                                                    @php
                                                        $unReadChatCount = $bid
                                                            ->chats()
                                                            ->where([
