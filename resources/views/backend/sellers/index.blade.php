...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        <div class="col-md-6">
            <h1 class="h3">{{translate('All Vendors/Sellers')}}</h1>
        </div>
    </div>
</div>
    ...
    <form class="" id="sort_sellers" action="" method="GET">
        <div class="card-header row gutters-5">
            <div class="col mb-2 mb-lg-0">
                <h5 class="mb-md-0 h6">{{ translate('Vendors/Sellers') }}</h5>
            </div>
            ...
            <div class="col-md-3">
                <div class="form-group mb-0">
                  <input type="text" class="form-control" id="search" onblur="sort_sellers()" name="search"@isset($sort_search) value="{{ $sort_search }}" @endisset placeholder="{{ translate('Type seller name or email & Enter') }}">
                </div>
            </div>
        </div>
        ...
        <div class="card-body">
            <table class="table aiz-table mb-0 table-responsive">
                ...
                <tr>
                    ...
                    <th>{{translate('Seller Code')}}</th>
                    <th>{{translate('Seller Name')}}</th>
                    <th>{{translate('Shop Name')}}</th> 
                    <th data-breakpoints="lg">{{translate('Shop Phone')}}</th>
                    <th data-breakpoints="lg">{{translate('Shop Email Address')}}</th>
                    <th data-breakpoints="lg">{{translate('Seller Verification Info')}}</th>
                        ...
                        <td>
                            @if(auth()->user()->can('delete_seller'))
                                <div class="form-group">
                                    <div class="aiz-checkbox-inline">
                                        <label class="aiz-checkbox">
                                            <input type="checkbox" class="check-one" name="id[]" value="{{encrypt($shop->id)}}">
                                            <span class="aiz-square-check"></span>
                                        </label>
                                    </div>
                                </div>
                        ...
                        <td>
                            @if($shop->verification_info != null) 
                                ...
                                @if($shop->verification_status)
                                    <span class="badge badge-inline badge-success">Verified <i class=""></i></span>
                                @endif
                            @endif
                        </td>
                        <td>
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-xs">
                                    ...
                                    @can('ban_seller')
                                        @if($shop->user->banned != 1)
                                            <a href="#" onclick="confirm_ban('{{route('sellers.ban', encrypt($shop->id))}}');" class="dropdown-item">
                                                {{translate('Ban this seller')}}
                                                <i class="fa fa-ban text-danger" aria-hidden="true"></i>
                                            </a>
                                        @else
                                            <a href="#" onclick="confirm_unban('{{route('sellers.ban', encrypt($shop->id))}}');" class="dropdown-item">
                                    ...
                                    @endcan
                                    @can('delete_seller')
                                        <a href="#" class="dropdown-item confirm-delete" data-href="{{route('sellers.destroy', encrypt($shop->id))}}" class="">
                                            {{translate('Delete')}}
                                        </a>
                                    @endcan
                                </div>
