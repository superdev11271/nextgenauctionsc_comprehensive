        ...
        @can('add_blog')
            <div class="col text-right">
                <a href="{{ route('blog.create') }}" class="btn btn-circle btn-info">
                    <span>{{translate('Add New News')}}</span>
                </a>
            </div>
        @endcan
    </div>
            ...
            <table class="table mb-0 aiz-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            @can('edit_blog')
                                <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{ route('blog.edit',encrypt($blog->id))}}" title="{{ translate('Edit') }}">
                                    <i class="las la-pen"></i>
                                </a>
                            @endcan
                            @can('delete_blog')
                                <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('blog.destroy', encrypt($blog->id))}}" title="{{ translate('Delete') }}">
