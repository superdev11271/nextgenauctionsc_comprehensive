                ...
                @foreach($staffs as $key => $staff)
                    @if($staff->user != null)
                        <tr>
                            ...
                            <td class="text-right">
                                @can('edit_staff')
                                    <a class="btn btn-soft-primary btn-icon btn-circle btn-sm mb-1 mb-lg-0" href="{{route('staffs.edit', encrypt($staff->id))}}" title="{{ translate('Edit') }}">
                                        <i class="las la-edit"></i>
                                    </a>
                                @endcan
                                @can('delete_staff')
                                    <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete mb-1 mb-lg-0" data-href="{{route('staffs.destroy', encrypt($staff->id))}}" title="{{ translate('Delete') }}">
