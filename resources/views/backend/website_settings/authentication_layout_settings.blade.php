	...
	<div class="card-body p-2rem">
		<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
			...
			<div class="row">
				...
				<div class="col-xxl-3 col-lg-4 col-sm-6 my-3">
					<label class="aiz-megabox d-block mb-3">
						<input value="xt-auth" type="radio" name="authentication_layout_select" @if($authentication_layout == 'xt-auth') checked @endif>
						<span class="d-block aiz-megabox-elem rounded-0 img-overlay">
							<div class="h-190px w-100 overflow-hidden">
								<img src="{{ static_asset('xt-assets/xt_auth_loin.png') }}" class="w-100" alt="authentication-page">
							</div>
						</span>
					</label>
					<div class="d-flex flex-wrap justify-content-between align-items-center">
						<span class="fs-14 fw-500 text-dark">{{ translate('Authentication Layout 4 - Xt-Auth') }}</span>
						<span>
							<a href="javascript:void(0);" class="btn btn-xs btn-danger rounded-0"
								onclick="imageShowOverlay('{{ static_asset('xt-assets/xt_auth_loin.png') }}')">{{ translate('View') }}</a>
						</span>
					</div>
				</div>
			</div>
