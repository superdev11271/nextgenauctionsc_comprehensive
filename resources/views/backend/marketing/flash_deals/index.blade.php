...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0 h6">{{translate('Flash Deals2')}}</h5>
            ...
            <tbody>
                @foreach($flash_deals as $key => $flash_deal)
                    <tr>
						...
						<td class="text-right">
                            @can('edit_flash_deal')
                                <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('flash_deals.edit', ['id'=>encrypt($flash_deal->id), 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
                                    <i class="las la-edit"></i>
                                </a>
                            @endcan
                            @can('delete_flash_deal')
                                <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('flash_deals.destroy', encrypt($flash_deal->id))}}" title="{{ translate('Delete') }}">
