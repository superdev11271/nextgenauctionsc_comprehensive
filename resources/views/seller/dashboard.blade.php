...
@section('panel_content')
    ...
    @if (Auth::user()->shop?->verification_status == 0)
            @if(Auth::user()->shop?->rejected ==1)
                ...
                {{__('Your Verification form process is rejected!')}}
                </div>
                @elseif(Auth::user()->shop?->verification_info)
                {{-- Panding --}}
                <div class="alert alert-warning" role="alert">
                {{__('Wait Your Verification process is pending!')}}
                @if(Auth::user()->shop?->remark)
                    <hr>
                    <small class="mt-2 text-danger">{{Auth::user()->shop?->remark}} </small>
                @endif
                </div>
                ...
                {{__('Please fill out the verification form first by clicking the verify button.')}}
                </div>
            @endif
    @endif
                ...
                <div class="card-body">
                    <div class="row align-items-center">
                        <div class="col">
                            <p class="small text-muted mb-0">
                                <span class="fe fe-arrow-down fe-12"></span>
                                <span class="fs-14 text-light">{{ translate('Total Deliverd Order') }}</span>
                            </p>
                            <h3 class="mb-0 text-white fs-30">
                ...
                <div class="card-body">
                    <div class="row align-items-center">
                        <div class="col">
                            <p class="small text-muted mb-0">
                                <span class="fe fe-arrow-down fe-12"></span>
                                <span class="fs-14 text-light">{{ translate('Total Pending Order') }}</span>
                            </p>
                            <h3 class="mb-0 text-white fs-30">
    ...
    <div class="row">
        ...
        <div class="col-sm-6 col-md-6 col-lg-3 mb-4">
            <div class="card shadow-none h-450px mb-0 h-100">
                <div class="card-body">
                    ...
                    <hr>
                    <div class="overflow-auto" style="max-height: 463px;">
                        <ul class="list-group">
        ...
        <div class="col-sm-6 col-md-6 col-lg-3 mb-4">
            @if (addon_is_activated('seller_subscription') && auth()->user()->shop?->verification_status == 1)
                @php
                                ...
                                <div class="col-9">
                                    ...
                                    <h6 class="text-primary">
                                        ...
                                        <p class="text-muted mb-4">{{ translate('Package Expires at') }}:
                                            ...
                                            @if ($isExpired)
                                                <a href="#" class="btn btn-soft-danger btn-sm">{{__('Expired')}}</a>
                                            @elseif($daysRemaining == 0)
                                                <a href="#" class="btn btn-soft-warning btn-sm">{{__('Expire Today')}}</a>
                                            @else
                                                <a href="#" class="btn btn-soft-success btn-sm">{{ $daysRemaining }} {{__('Days remaining')}} </a>
                                            @endif
                                        </p>
                                        <div class="">
            ...
            <div
                ...
                @if (Auth::user()->shop?->verification_status == 0)
                  @if(Auth::user()->shop?->rejected ==1)
                    <img  width="200px" src="{{ static_asset('assets/img/rejected.png') }}" alt="">
                    <a href="{{ route('seller.shop.verify') }}"
                            class="btn btn-sm btn-primary">{{ translate('Verify Now') }}</a>
                  @elseif(Auth::user()->shop?->verification_info)
                    <img  width="200px" src="{{ static_asset('assets/img/pending.png') }}" alt="">
                    @if(Auth::user()->shop?->remark)
                    <a href="{{ route('seller.shop.verify') }}"
                            class="btn btn-sm btn-primary">{{ translate('Verify Now') }}</a>
                    <small class="mt-2 text-danger">{{Auth::user()->shop?->remark}} </small>
                  ...
                  @else
                    <div class="my-n4 py-1 text-center">
                        <img  src="{{ static_asset('assets/img/non_verified.png') }}" alt=""
                            class="w-xxl-140px w-100px d-block">
                        <a href="{{ route('seller.shop.verify') }}"
                            class="btn btn-sm btn-primary">{{ translate('Verify Now') }}</a>
                    </div>
                  @endif
                @else
                    <div class="my-2 py-1">
                        <img src="{{ static_asset('assets/img/verified1.png') }}" alt="" width="">
                    </div>
                @endif
