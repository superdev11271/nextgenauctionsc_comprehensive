...
@section('panel_content')
    ...
    <div class="card">
        ...
        <div class="card-body">
                ...
                <div class="col-md-4 ml-auto">
                    <table>
                        <tbody>
                            ...
                            <tr>
                                <td class="text-main text-bold">{{ translate("Sellers' Premium") }}</td>
                                <td class="text-right">{{ $order->seller->shop?->commission ?? get_setting('vendor_commission') }}%</td>
                            </tr>
                    ...
                    <table class="table-bordered aiz-table invoice-summary table">
                        ...
                        <tbody>
                            @foreach ($order->orderDetails as $key => $orderDetail)
                              @php $data = json_decode($orderDetail->data) @endphp
                              @php
                              ...
                              @endphp
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>
                                        @if ($orderDetail->data != null && $orderDetail->product->auction_product == 0)
                                            @if(is_array($data))
                                                @if($product_stock?->image)
                                                <img height="50"
                                                    src="{{ uploaded_asset($product_stock->image) }}">
                                                @else
                                                <img height="50"
                                                    src="{{ uploaded_asset(json_decode($orderDetail->data[0])->product->photos) }}">
                                                @endif
                                            @else
                                                <img height="50"
                                                    src="{{ uploaded_asset($data->product->photos) }}">
                                            @endif
                                        @elseif ($orderDetail->data != null && $orderDetail->product->auction_product == 1)
                                            @if(is_array($data))
                                                <img height="50"
                                                    src="{{ uploaded_asset(json_decode($orderDetail->data[0])->product->photos) }}">
                                            @else
                                    ...
                                    <td>
                                        @if ($orderDetail->data != null && $orderDetail->product->auction_product == 0)
                                            ...
                                            @if(is_array($data))
                                                <strong>{{json_decode($orderDetail->data[0])->product->name}}
                                                </strong>
                                    ...
                                    <td class="text-center">
                                        {{ single_price($orderDetail->tax) }}</td>
                                    <td class="text-center">
                                        {{ single_price($orderDetail->price / $orderDetail->quantity) }}</td>
                                    <td class="text-center">{{ single_price($orderDetail->price + $orderDetail->tax) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            <div class="clearfix float-right w-100 w-lg-auto">
                <table class="table">
                    <tbody>
                        <tr>
                            <td>
                ...
                <div class="no-print text-right">
                    <a href="{{ route('seller.invoice.download', encrypt($order->id)) }}" type="button"
