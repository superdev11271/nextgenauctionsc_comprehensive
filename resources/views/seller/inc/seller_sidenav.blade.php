    ...
    <div class="aiz-sidebar left c-scrollbar">
        <div class="aiz-side-nav-logo-wrap">
            <div class="d-block text-center my-3">
                @if (optional(Auth::user()->shop)->logo != null)
                    <img class="mw-100 mb-3 brand-icon" src="{{ uploaded_asset(optional(Auth::user()->shop)->logo) }}"
                        alt="{{ get_setting('site_name') }}">
                @else
                    <img class="mw-100 mb-3 brand-icon" src="{{ uploaded_asset(get_setting('system_logo_black')) }}"
                        alt="{{ get_setting('site_name') }}ghgfhhh">
                @endif
                <h3 class="fs-16  m-0 text-primary">{{ optional(Auth::user()->shop)->name }}</h3>
                <p class="text-primary">{{ Auth::user()->email }}</p>

                @if(Auth::user()->user_code != null)
                    <p class="text-primary">({{ Auth::user()->user_code }})</p>
                @endif
            </div>
        </div>
        <div class="aiz-side-nav-wrap">
            ...
            @if(Auth::user()->shop?->verification_status == 1)
                ...
                @if (addon_is_activated('auction') && get_setting('seller_auction_product') == 1)
                    <li class="aiz-side-nav-item">
                        ...
                        <ul class="aiz-side-nav-list level-2">
                            ...
                            <li class="aiz-side-nav-item">
                                <a href="{{ route('current_bid.seller') }}"
                                    class="aiz-side-nav-link {{ areActiveRoutes(['current_bid.seller']) }}">
                                    <span class="aiz-side-nav-text">{{ translate('Live Auction Bid List') }}</span>
                                </a>
                            </li>

                            <li class="aiz-side-nav-item">
                                <a href="{{ route('seller.aucton_product_bulk_upload') }}"
                                    class="aiz-side-nav-link {{ areActiveRoutes(['seller.aucton_product_bulk_upload']) }}">
                                    <span class="aiz-side-nav-text">{{ translate('Auction Product Bulk Upload') }}</span>
                                </a>
                            </li>
                        </ul>
                    </li>
                ...
                {{-- <li class="aiz-side-nav-item">
                    <a href="{{ route('seller.orders.my_orders') }}"
                        class="aiz-side-nav-link {{ areActiveRoutes(['seller.orders.my_orders', 'seller.orders.show']) }}">
                ...
                <li class="aiz-side-nav-item">
                    <a href="{{ route('seller.shop.index') }}"
                        ...
                        <span class="aiz-side-nav-text">{{ translate('Business Setting') }}</span>
                    </a>
                </li>
                ...
                <li class="aiz-side-nav-item">
                    <a href="{{ route('seller.payments.index') }}"
                        ...
                        <span class="aiz-side-nav-text">{{ translate('Withdrawal History') }}</span>
                    </a>
                </li>
