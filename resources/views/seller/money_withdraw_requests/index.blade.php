...
@section('panel_content')
    ...
    @if(!(auth()->user()->shop?->cash_on_delivery_status || auth()->user()->shop?->bank_payment_status))
        <div class="alert alert-danger" role="alert">
            {{ translate('Payment method not configured. Please configure it.') }}
        </div>
    @endif
    <div class="row gutters-10">
        ...
        <div class="col-md-4 mb-3 mr-auto" >
          <div class="p-3 rounded mb-3 c-pointer text-center bg-white shadow-sm hov-shadow-lg has-transition" @if((auth()->user()->shop?->cash_on_delivery_status || auth()->user()->shop?->bank_payment_status)) onclick="show_request_modal()"@else onclick="show_alert();" @endif>
...
@section('script')
    <script type="text/javascript">
        ...
        function show_alert(){
            Swal.fire({
                title: "{{__('Withdrawal Error!')}}?",
                html: '<p class="text-danger">{{ __("Please configure your payment details") }} <br><small><a href="{{ route('seller.profile.index') }}">Configure now</a></small></p>',
                icon: "warning",
