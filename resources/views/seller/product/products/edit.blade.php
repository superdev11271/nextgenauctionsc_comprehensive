...
<form class="" action="{{route('seller.products.update', $product->id)}}" method="POST" enctype="multipart/form-data"
    ...
    <div class="tab-content">
                ...
                <div class="row">
                    <div class="col-xxl-7 col-xl-6">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-xxl-9">
                                <input type="text" oninput="this.value=this.value.replace(/[^A-Za-z\s^0-9]/g,'')" id="name" class="form-control" name="name"
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input type="text" placeholder="{{ translate('Unit price') }}"
                                    name="unit_price" id="unit_price" class="form-control" value="{{ $product->unit_price }}">
                        ...
                        <!-- SKU -->
                        {{-- <div class="form-group row">
                            <label class="col-xxl-3 col-from-label">
                                {{ translate('SKU') }}
                            </label>
                            <div class="col-xxl-9">
                                <input type="text" placeholder="{{ translate('SKU') }}"
                                    value="{{ old("sku",optional($product->stocks->first())->sku) }}" name="sku"
                                    class="form-control">
                            </div>
                        </div>
                        <!-- Unit -->
                            ...
                            <div class="col-xxl-9">
                                <select name="unit" id="unit" class="form-control js-example-tags">
                                    ...
                                    @foreach($units as $unit)
                                        <option value="{{ $unit->name }}" {{ old('unit', $product?->productUnit?->name) == $unit->name ? 'selected' : '' }}>{{ $unit->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-xxl-9">
                                <input type="number" class="form-control" name="weight"
                                    value="{{ old("weight",$product->weight) }}" step="0.01" placeholder="0.00">
                            </div>
                        </div>
                        <!-- Quantity -->
                        <div class="form-group row">
                            ...
                            <div class="col-xxl-9">
                                @php
                                    $min_qty = $product->min_qty <= 0 ? 1 :$product->min_qty
                                @endphp
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input type="checkbox" onchange="toggleTaxSection(this)" name="tax_enabled" value="1" @checked($product->hasTaxes())>
                                    <span></span>
                                </label>
                            </div>
                        </div>
                        <div class="w-100 tax-section {{$product->hasTaxes()?'':'d-none'}}">
                    ...
                    <div class="col-xxl-7 col-xl-6">
                        <div class="form-group row">
                            ...
                            <div class="col-xxl-9">
                                <input type="text" class="form-control" name="expiry_date" placeholder="DD-MM-YYYY HH:MM" id="expirydate" 
                                    value="{{  $product->auction_end_date ? \Carbon\Carbon::parse($product->auction_end_date)->format('d-m-Y H:i') : '' }}">
                                
                                @if($product->auction_end_date)
                                    <span class="text-danger ml-1">
                                        {{ translate('This product will expire at') }} 
                                        {{ \Carbon\Carbon::parse($product->auction_end_date)->format('F j, Y g:i A') }}
                                    </span>
                                @else
                                    <span class="text-danger ml-1">{{ translate('No expiry set currently!') }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div class="w-100">
                ...
                <div class="form-group row">
                    <label class="col-sm-3 control-label"
                        for="start_date" data-toggle="tooltip" title="If you provide a date range, the discount will be applied only within the given dates." >{{ translate('Select date range for discount') }}
                        <label class="aiz-switch aiz-switch-success mb-0 float-right"  data-toggle="tooltip" title="Show date range">
                            <input type="checkbox"  value="" @if ($product->discount_start_date && $product->discount_end_date) @checked('1')  @endif id="show-date-range">
                            <span></span>
                        </label>
                    </label>
                    <div class="col-sm-6 @if(!($product->discount_start_date && $product->discount_end_date)) d-none  @endif date-range">
        ...
        <div class="bg-white p-3 p-sm-2rem">
            ...
            {{-- <h5 class="mb-3 mt-4 pb-3 fs-17 fw-700" id="shipping_days_heading" style="border-bottom: 1px dashed #e4e5eb;display:none">
            ...
            <div class="w-100" id="pickup_section" >
                <div class="form-group row">
                    ...
                    <div class="col-md-9">
                        <div class="input-group">
                            <input type="text" class="form-control" id="pickdate" name="pickup_days"  required value="{{ $product->pickup_days }}" placeholder="{{ translate('Pickup Date') }}">
                            <div class="input-group-prepend">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-9">
                        <div class="input-group">
                            <input type="text" class="form-control" value="{{ $product->pickup_time }}" name="pickup_time" id="picktime" required  />
                            <div class="input-group-prepend">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-9">
                        <div class="input-group">
                            <input type="text" class="form-control " value="{{ $product->pickup_address }}" name="pickup_address" required    />
                            <div class="input-group-prepend">
...
<script type="text/javascript">
        ...
        let expiryDate = "{{ $product->auction_end_date ? \Carbon\Carbon::parse($product->auction_end_date)->format('d-m-Y H:i') : '' }}";
        ...
        $('#expirydate').daterangepicker({
            ...
            startDate: expiryDate ? moment(expiryDate, 'DD-MM-YYYY HH:mm') : moment(),  // Use current moment if no expiryDate
            autoUpdateInput: false,
            minDate: moment(),
        }, function(start) {
            // Update the input field when a date is selected
            $('#expirydate').val(start.format('DD-MM-YYYY HH:mm'));
        });

        // Pre-fill the input field if expiryDate exists
        if (expiryDate) {
            $('#expirydate').val(expiryDate);
        } else {
            $('#expirydate').val('');  // Keep the field blank if no expiry date is set
        }

        // Handle clear action when the cancel button is clicked
        $('#expirydate').on('cancel.daterangepicker', function(ev, picker) {
            $(this).val('');  // Clear the input field when 'Clear' is clicked
        });
