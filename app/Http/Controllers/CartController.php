...
class CartController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('frontend.xt-user.xt_view_cart', compact('carts'));
    ...
    {
        $product = Product::find($request->id);
        return view('auction.frontend.xthome.xt-addToCartAuction', compact('product'));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $cart->shipping_type = getDeliveryType($product->id);
        ...
        return array(
            'status' => 1,
            'cart_count' => count($carts),
            'modal_view' => view('frontend.'.get_setting('homepage_select').'.partials.addedToCart', compact('product', 'cart','product_stock'))->render(),
            'nav_cart_view' => view('frontend.'.get_setting('homepage_select').'.partials.cart')->render(),
            'xt_cart_view' =>  view('frontend.'.get_setting('homepage_select').'.partials.xt-cart-detail',compact('product', 'cart','product_stock'))->render(),
    ...
    public function removeFromCart(Request $request)
    {
        ...
        return array(
            ...
            'nav_cart_view' => view('frontend.'.get_setting('homepage_select').'.partials.xt-cart-detail')->render(),
        );
    }
    ...
    public function updateQuantity(Request $request)
    {
        ...
        if ($cartItem['id'] == $request->id) {
            ...
            $subtotal = (floatval($cartItem->price) + floatval($cartItem->tax)) * intval($cartItem->quantity);
        }
        ...
        return array(
            'cart_count'       => count((array)$carts),
            'cart_id'          => $cartItem['id'] ?? 0,
            'product_id'       => $product->id ?? 0,
            'qty'              => intval($cartItem['quantity']) ?? 0,
            'product_subtotal' => single_price($subtotal),
            'user_cart_total'  => get_user_cart_total(),
            'nav_cart_view' => view('frontend.'.get_setting('homepage_select').'.partials.xt-cart-detail')->render(),
        );
    }
}
