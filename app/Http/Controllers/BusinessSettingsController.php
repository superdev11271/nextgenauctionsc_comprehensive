...
class BusinessSettingsController extends Controller
{
    ...
    public function payment_method_update(Request $request)
    {
        ...
        if ($request->payment_method == 'paypal') {
            ...
            $paypal_payment->value = $request->paypal_payment ?? 0;
            $paypal_payment->save();
        } else {
            $stripe_payment = BusinessSetting::where('type', 'stripe_payment')->first();
            $stripe_payment->value = $request->stripe_payment ?? 0;
            $stripe_payment->save();
        }
    ...
    {
        $business_settings = BusinessSetting::where(["type"=>decrypt($request->setting)])->firstOrFail();
        $business_settings->value = $request->value;
        Artisan::call('cache:clear');
        if($business_settings->save()) return 1;
        return 0;
    }

    public function LiveAuctionSettings(Request $request)
    {
        return view('backend.system.live_auction_settings');
    }

    public function bulk_updates(Request $request)
    {
        foreach ($request->business_settings as $settingType => $settingValue){
    ...
    public function vendor_commission(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $user_ids = User::whereIn('user_type', ['seller', 'customer'])->where(function ($user) use ($sort_search) {
        ...
        return view('backend.sellers.seller_commission.index', compact('shops', 'approved', 'sort_search'));
    }
    ...
    public function vendor_commission_update(Request $request)
    {
        ...
        if (in_array('minimum_seller_amount_withdraw', $request->types ?? [])) {
            flash(translate('Minimum withdrawal amount updated successfully.'))->success();
        } else {
            flash(translate('Default commission (%) updated successfully.'))->success();
        }
        return back();
    }
