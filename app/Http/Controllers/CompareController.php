...
class CompareController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('frontend.xt-user.xt_view_compare', compact('categories'));
    }
    ...
    public function addToCompare(Request $request)
    {
        if($request->session()->has('compare')){
            ...
            if(!$compare->contains($request->id)){
                ...
                $id = $request->id;
                $compareList = $request->session()->get('compare', collect([]));
                $compareList = $compareList->reject(function ($item) use ($id) {
                    return $item == $id;
                });
    ...
    public function remove(Request $request, $product)
    {
        ...
        $compareList = $compareList->reject(function ($item) use ($productId) {
