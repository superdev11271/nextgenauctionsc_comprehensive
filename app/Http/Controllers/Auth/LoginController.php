...
class LoginController extends Controller
{
    ...
    */
    ...
    public function authenticated()
    {
        ...
        if (request()->ajax()) {
            if(auth()->user()->banned ==1){
                auth()->guard()->logout();
                return response()->json(['error' => translate('You are not authorise to login...')]);
            }

            if (auth()->user()->user_type == 'admin' || auth()->user()->user_type == 'staff') {
                $redirect_link = route('admin.dashboard');
            } elseif (auth()->user()->user_type == 'seller' && auth()->user()->shop) {
                $redirect_link = route('seller.dashboard');
            }elseif(auth()->user()->user_type == 'customer' && auth()->user()->shop) {
                if (session('link') != null) {
                    $redirect_link = (session('link'));
                } else {
                    $redirect_link = route('dashboard');
                }
            } else {
                if (session('link') != null) {
                    $redirect_link = (session('link'));
            ...
            return response()->json(['success' => translate('Login successful'), 'link' => $redirect_link]);
        }
        if (auth()->user()->user_type == 'admin' || auth()->user()->user_type == 'staff') {
    ...
    {
        if ($request->ajax()) {
            return response()->json(['error' => translate('Invalid login credentials')]);
        }
