...
class CommissionController extends Controller
{
    ...
    public function pay_to_seller(Request $request)
    {
        try {
            ...
            if($request->withdraw_request_id){
                $data['withdraw_request_id'] = $request->withdraw_request_id;

                $seller_withdraw_request = SellerWithdrawRequest::findOrFail($request->withdraw_request_id);

                if ($seller_withdraw_request->status) {
                    flash(translate('You have already paid for this request!'))->error();
                    return redirect()->route('withdraw_requests_all');
                }

                if ($seller_withdraw_request->amount != $request->amount) {
                    flash(translate('The requested amount is not the same.'))->error();
                    return redirect()->route('withdraw_requests_all');
                }
    ...
    public function seller_payment_done($payment_data, $payment_details)
    {
        ...
        Notification::send($users, new PayoutNotification($shop->user, $payment_data['amount'], 'paid',$payment_data['txn_code']));
        ...
        if ($order->payment_type == 'cash_on_delivery') {
            foreach ($order->orderDetails as $orderDetail) {
                ...
                if (get_setting('vendor_commission_activation')) {
                    //This is check user is seller or Customer has shop or also check proudct must be auction products
                    if (($orderDetail->product->user->user_type == 'seller' || $orderDetail->product->user->shop) && $orderDetail->product->auction_product == 1) {
                        $commission_percentage = $orderDetail->product->user->shop?->commission ?? get_setting('vendor_commission');
                    }
                }
                if ($orderDetail->product->user->user_type == 'seller' || $orderDetail->product->user->shop) {
