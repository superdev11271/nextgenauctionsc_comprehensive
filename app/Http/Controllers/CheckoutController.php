...
class CheckoutController extends Controller
{
    ...
    public function checkout_done($combined_order_id, $payment=null )
    {
        ...
        foreach ($combined_order->orders as $key => $order) {
            ...
            // Start Queue if product is Auction Producte
            $bid = AuctionProductBid::where(["user_id"=>$user_id,"product_id"=>$product_id])->first();

            if($bid) XeroMarkInvoiceAsPaid::dispatch($bid->id);
    ...
    public function get_shipping_info(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            ...
            return view('frontend.xt-user.xt_shipping_info', compact('categories', 'carts'));
        }
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        if ($auth_user != null) {
            ...
            foreach ($carts as $key => $cartItem) {
                ...
                $cartItem->billing_address_id = $request->billing_address_id;
                $cartItem->save();
            }
        } else {
        ...
        return redirect()->route("checkout.payment-select");
    ...
    public function payment_select(Request $request)
    {
        $carts = auth()->user() != null ?
            Cart::query()->with("product")->where('user_id', auth()->user()->id)->get() :
        ...
        // $cartHasAuctionProduct = $carts->where("product",function($q){
        //     // dd($q);
        //     $q->where("auction_product",1);
        // })->dd();
        // dd($cartHasAuctionProduct);
        ...
        foreach ($carts as $key => $cartItem) {
            ...
            $cartItem['shipping_type'] = getDeliveryType($product->id);
            $shipping += $cartItem['shipping_cost'];
            $cartItem->save();
        }
        $total = $subtotal + $tax + $shipping;
        return view('frontend.xt-user.xt_payment_select', compact('carts', 'total', "tax", "shipping", "subtotal",));
    }
    ...
    public function apply_coupon_code(Request $request)
    {
        ...
        // dd($response_message, compact('coupon', 'carts'));
    ...
    public function order_confirmed()
    {
        ...
        $email = $combined_order->orders[0]->user->email;
        $order =  encrypt($combined_order->orders[0]->id);
        return view('frontend.xt-user.xt_order_confirmed'
        , compact('combined_order',"email","order")
    );
    }
