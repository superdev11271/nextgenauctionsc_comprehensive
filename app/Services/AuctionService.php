...
class AuctionService
{
    public function store(Request $request){
        ...
        if(Auth::user()->user_type == 'seller' || Auth::user()->shop?->verification_status==1){
        ...
        $product->auction_label     = $request->auction_label;
        $product->auction_number    = $request->auction_number;
        $product->estimate_start    = $request->estimate_start;
        $product->reserved_price    = $request->reserved_price;
        $product->estimate_end      = $request->estimate_end;
        ...
        $product->pickup_days       = $request->pickup_days;
        $product->pickup_time       = $request->pickup_time;
    ...
    public function update(Request $request , $id){
        ...
        $product->auction_number    = $request->auction_number;
        $product->auction_label     = $request->auction_label;
        syncProductColumn('auction_number', $product->auction_number, 'auction_label',$request->auction_label);
        syncProductColumn('auction_number', $product->auction_number, 'banner_image',$request->banner_image);
        $product->reserved_price    = $request->reserved_price;
        $product->estimate_start    = $request->estimate_start;
        $product->estimate_end      = $request->estimate_end;
